			...
			elseif ( $lesson_complete ) {
				...
				elseif ( isset( $user_lesson_status->comment_approved ) && 'failed' == $user_lesson_status->comment_approved ) {
					...
					if( $is_lesson ) {
						...
						$message = sprintf( __( 'You require %1$d%% to pass this quiz. Your grade is %2$s%% Watch again or reset the quiz.', 'woothemes-sensei' ),  Sensei_Utils::round( $quiz_passmark ),  Sensei_Utils::round( $quiz_grade ) );
                ...
                //echo $actual_link;
                $new_link = str_replace("quiz","lesson",$actual_link);
                //echo $new_link;
                //echo $nav_links['next']['url'];
                    $message .= ' ' . '<a class="next-lesson" href="' . esc_url( $new_link )
                                . '" rel="prev"><span class="meta-nav"></span>'. __( 'Watch Again?' ,'woothemes-sensei')
                                .'</a>';
